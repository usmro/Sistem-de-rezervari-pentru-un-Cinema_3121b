...
BiletConfigurabil::BiletConfigurabil(const std::string &numeTip,
...
double BiletFactory::calculeazaPret(const std::string &tip, double pretBaza,
                                    double multSala) {
...
BiletFactory::getTipuriPermise(TipSala tipSala) {
  if (!incarcat)
    incarcaPreturi();
