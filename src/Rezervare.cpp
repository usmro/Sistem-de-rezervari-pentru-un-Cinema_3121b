...
Rezervare::Rezervare(const std::string& idRezervare,
    ...
    : idRezervare(idRezervare), proiectie(std::move(proiectie)),
      rand(rand), loc(loc), tipBilet(tipBilet),
      pretFinal(pretFinal), usernameClient(usernameClient), anulata(false) {}
...
const std::string& Rezervare::getUsernameClient() const { return usernameClient; }
...
std::string Rezervare::toCSV() const {
    return idRezervare + "," +
           std::to_string(proiectie->getId()) + "," +
           std::to_string(rand) + "," +
           std::to_string(loc) + "," +
           tipBilet + "," +
           std::to_string(pretFinal) + "," +
           usernameClient + "," +
           (anulata ? "1" : "0");
}
