...
Voucher::Voucher(const std::string& cod, double reducereProcent,
                 ...
                 bool activ)
    : cod(cod), reducereProcent(reducereProcent),
      ziuaValida(ziuaValida), tipuriExcluse(tipuriExcluse), activ(activ) {}
...
const std::vector<std::string>& Voucher::getTipuriExcluse() const { return tipuriExcluse; }
...
bool Voucher::esteValid(const std::string& tipBilet) const {
    ...
    if (ziuaValida != ZiuaSaptamanii::Toate) {
        ...
        if (ziuaAzi != ziuaToInt(ziuaValida)) return false;
    }
    return true;
}
...
std::string Voucher::descriereZiua() const {
    return ziuaToString(ziuaValida);
}
std::string Voucher::toCSV() const {
    ...
    if (excl.empty()) excl = "-";
    return cod + "," +
           std::to_string(reducereProcent) + "," +
           std::to_string(ziuaToInt(ziuaValida)) + "," +
           excl + "," +
           (activ ? "1" : "0");
}
Voucher Voucher::fromCSV(const std::string& linie) {
    ...
    if (excluseStr != "-" && !excluseStr.empty()) {
        ...
        while (std::getline(ssE, tip, '|')) {
            if (!tip.empty()) excl.push_back(tip);
        }
    }
    return Voucher(codStr, reduc, ziua, excl, act);
}
int Voucher::ziuaToInt(ZiuaSaptamanii z) {
    return static_cast<int>(z);
}
ZiuaSaptamanii Voucher::intToZiua(int i) {
    if (i == -1) return ZiuaSaptamanii::Toate;
    if (i >= 0 && i <= 6) return static_cast<ZiuaSaptamanii>(i);
    return ZiuaSaptamanii::Toate;
}
std::string Voucher::ziuaToString(ZiuaSaptamanii z) {
    switch (z) {
        case ZiuaSaptamanii::Toate:    return "Toate zilele";
        case ZiuaSaptamanii::Luni:     return "Luni";
        case ZiuaSaptamanii::Marti:    return "Marti";
        case ZiuaSaptamanii::Miercuri: return "Miercuri";
        case ZiuaSaptamanii::Joi:      return "Joi";
        case ZiuaSaptamanii::Vineri:   return "Vineri";
        case ZiuaSaptamanii::Sambata:  return "Sambata";
        case ZiuaSaptamanii::Duminica: return "Duminica";
    }
    return "Toate zilele";
}
